...
Adapted to work with differing sizes, an arbitrary color, and thickness from the
Glowing line brush in the OF Book.
*/
void DrawWithBubbleBrush(int thickness, ofColor color) {
    for (int radius = thickness; radius > 0; radius -= kRadiusStepSize) {
        // We put the circles in a circle using polar coordinates, so convert and
        // convert those coordinates into radians.
        float angle = ofRandom(ofDegToRad(360.0));
        float distance = ofRandom(thickness);
        float xOffset = cos(angle) * distance;
        float yOffset = sin(angle) * distance;

        // For every circle, randomly select a color between the selected one and
        // a dimmer version of the selected one.
        ofColor darkened_color = ofColor(color.r * kDimmingFactor,
          color.g * kDimmingFactor, color.b * kDimmingFactor, color.a);
...
void PreviewBrush(int thickness, ofColor color, Brushes active_brush,
  ofColor background_color) {
    ...
    ofSetColor(black);
    ofDrawCircle(ofGetMouseX(), ofGetMouseY(), kOutlineRatio * thickness);
